...
async function editCategory(oldName){
  const newName=prompt('Nuevo nombre:',oldName)?.trim(); if(!newName||newName===oldName)return;if(store.categories.includes(newName)){alert('Esa categoría ya existe');return;}for(const p of store.products.filter(p=>p.category===oldName)){await saveProductToFB({...p,category:newName});}await saveCategoriesToFB(store.categories.map(c=>c===oldName?newName:c));
}
